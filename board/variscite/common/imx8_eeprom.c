...
#elif CONFIG_TARGET_IMX8MP_VAR_DART || CONIFG_TARGET_IMX8MP_VAR_SOM_PILATUS
...
# else
	...
	for (i = 0; i < adj_table_size; i++) {
		...
		dm_i2c_read(dev, off, (uint8_t *)&adj_table_row, sizeof(adj_table_row));
#else
...
void var_eeprom_adjust_dram(struct var_eeprom *ep, struct dram_timing_info *d)
{
	...
	adjust_dram_table(ep->off[0], adj_table_size[0], d->ddrc_cfg, d->ddrc_cfg_num);

	/* Adjust DDR PHY configuration table */
	debug("\nAdjusting DDR PHY CFG table: offset=%d, count=%d\n",
	      ep->off[1], adj_table_size[1]);
	adjust_dram_table(ep->off[1], adj_table_size[1], d->ddrphy_cfg, d->ddrphy_cfg_num);

	/* Adjust DDR PHY PIE table */
	debug("\nAdjusting DDR PHY PIE table: offset=%d, count=%d\n",
	      ep->off[2], adj_table_size[2]);
	adjust_dram_table(ep->off[2], adj_table_size[2], d->ddrphy_pie, d->ddrphy_pie_num);
