...
#define BACKLIGHT_PAD_CTRL (PAD_CTL_PUS_100K_DOWN |		\
	PAD_CTL_SPEED_MED | PAD_CTL_DSE_40ohm | PAD_CTL_HYS)
...
static bool is_som_solo(void)
{
	...
	return (0 != dm_i2c_chip_probe(PMIC_I2C_BUS, CFG_POWER_PFUZE100_I2C_ADDR));
}
...
static bool is_som_solo(void)
{
	...
	/* Probing for PMIC which is preset on all SOM types but SOM-SOLO */
	ret = (0 != i2c_probe(CFG_POWER_PFUZE100_I2C_ADDR));
...
int board_early_init_f(void)
{
	...
	SETUP_IOMUX_PAD(PAD_DISP0_DAT9__GPIO4_IO30 | MUX_PAD_CTRL(BACKLIGHT_PAD_CTRL));
...
static int power_init_pmic_sw2(void)
{
	if (!is_som_solo() && !is_dart_board()) {
		...
		/* Set SW2 to 3.2V */
		if (i2c_read(CFG_POWER_PFUZE100_I2C_ADDR, PFUZE100_SW2VOL, 1, &reg, 1))
			return -1;

		reg &= ~0x7f;
		reg |= 0x70; /* SW2x 3.2V */

		if (i2c_write(CFG_POWER_PFUZE100_I2C_ADDR, PFUZE100_SW2VOL, 1, &reg, 1))
			return -1;
	}
